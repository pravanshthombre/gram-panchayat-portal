...
router.post('/signup', async (req, res) => {
  try {
    ...
    const existing = await prepareGet('SELECT id FROM users WHERE email = ?', email);
    if (existing) return res.status(409).json({ error: 'Email already registered.' });
    const hash = bcrypt.hashSync(password, 10);
    const result = await runSql('INSERT INTO users (name, email, password, role, village_id) VALUES (?, ?, ?, ?, ?)', name, email, hash, role || 'villager', village_id || null);
    const user = await prepareGet('SELECT id, name, email, role, village_id FROM users WHERE id = ?', result.lastInsertRowid);
...
router.post('/login', async (req, res) => {
  try {
    ...
    const user = await prepareGet('SELECT * FROM users WHERE email = ?', email);
...
router.get('/me', authenticateToken, async (req, res) => {
  try {
    const user = await prepareGet('SELECT id, name, email, role, village_id, created_at FROM users WHERE id = ?', req.user.id);
